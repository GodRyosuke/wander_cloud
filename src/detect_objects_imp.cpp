...
// 注意
// ダウンサンプリングする数字が大きすぎるとRANSACにより形状を認識
// してもらえなくなる
...
{
    if (!detect_intersect_circle_at_xy()) {
        return;
    }
...
bool detect_objects::detect_intersect_circle_at_xy()
{
    ...
    bool have_nan = false;
    for (int i = 0; i < 4; i++) {
        if (!std::isfinite(coefficients_plane->values[i])) {
            have_nan = true;
        }
    }
    if (have_nan) {
        ROS_INFO("error: cannnot detect plane!!");
        return false;
    }
